...
import Control.Exception (SomeException(..), catchJust, catch, throw)
...
emulatorFallback fallback first_try = catch first_try (\e -> let _ = (e :: ConsoleException) in fallback)
